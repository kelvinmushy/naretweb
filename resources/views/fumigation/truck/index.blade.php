...
@section('bot')
    ...
    <script type="text/javascript">
        ...
        function addForm() {
            ...
            $('.modal-title').text('Truck Details');
        ...
        function editForm(id) {
            ...
            $('#modal-form form')[0].reset();
            $.ajax({
                url: "{{ url('truck') }}" + '/' + id + "/edit",
                type: "GET",
                dataType: "JSON",
                success: function(html) {
                    $('#modal-form').modal('show');
                    $('.modal-title').text('Edit Truck Dtails');
                    $('#id').val(html.data.id);
                    $('#company_id').val(html.data.company_id);
                    $('#truck_name').val(html.data.truck_name);
                    $('#truck_number').val(html.data.truck_number);
        ...
        $(function(){
            $('#modal-form form').validator().on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('truck') }}";
                    else url = "{{ url('truck') . '/' }}" + id;
