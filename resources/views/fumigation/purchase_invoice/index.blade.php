...
@section('content')
    ...
    @include('fumigation.purchase_invoice.form');

    @include('fumigation.purchase_payment.pay');
...
@include('fumigation.expensive.form');
    ...
    <script type="text/javascript">
         var table = $('#purchase-table').DataTable({
            processing: true,
            serverSide: true,
            ajax:  "{{ url('apiPurchase1') }}",
            columns: [
                {data: 'name', name: 'name'},
                {data: 'tin_number', name: 'tin_number'},
                {data: 'purchase_invoice1', name: 'purchase_invoice1'},
...
$('#net_total').val(net);
 $('#amount_due').val(sub); 
  ...
  function editForm(id) {
            ...
            count_charge=''; 
            $.ajax({
                url: "{{ url('/purchase_invoice1') }}" + '/' + id + "/edit",
                type: "GET",
                dataType: "JSON",
                success: function(html) {
                    $('#modal-form').modal('show');
        ...
        $(function(){
            $('#modal-form form').validator().on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('purchase_invoice1') }}";
                    else url = "{{ url('purchase_invoice1') . '/' }}" + id;
        ...
        $(function(){
            $('#form-item-payment').validator().on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    ...
                    if (save_method == 'add') url = "{{ url('purchase_history1') }}";
